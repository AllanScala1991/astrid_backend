...
export class TaskController implements ITask {
    ...
    async create(task: IRegisterTask): Promise<ITaskResponse> {
        if(!task.name || !task.stageId) return {status: false, message: "Campos obrigatórios devem ser preenchidos.", statusCode: 400}
    ...
    async findByTaskId(taskId: string): Promise<ITaskResponse> {
        if(!taskId) return {status: false, message: "ID da tarefa inválido.", statusCode: 400}
